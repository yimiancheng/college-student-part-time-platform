...
strVarCity += '    <table class="aui_border" style="border:0px solid #fff;">';
strVarCity += '      <tbody>';
strVarCity += '        <tr>';
...
strVarCity += '                <tbody>';
strVarCity += '                  <tr>';
strVarCity += '                    <td class="aui_header" colspan="2" style="background-color:#0062c8;"><div class="aui_titleBar">';
strVarCity += '                      <div class="aui_title" style="cursor: move;color: #fff;">选择地域</div>';
...
strVarCity += '                          <div id="" style="width: 720px; position:relative;">';
...
strVarCity += '                      <button class="aui-btn aui-btn-primary"' +
                                        ' style="background:#0062c8!important;color:#fff;" onclick="svae_City()" type="button">确定</button>';
...
// 全局变量
var DATA_INPUT_TYPE = "";
var DATA_CITY_INPUT = null;

$(document).on('click', '#adCitys', function () {
    appendCity(this, 'duoxuan');
});

function appendCity(thiscon, Cityxz) {
    DATA_CITY_INPUT = thiscon;
    DATA_INPUT_TYPE = Cityxz;
    $('body').append(strVarCity);
    if (DATA_INPUT_TYPE == "danxuan") {
        $('.data-result').find('strong').text('1');
    } else {
        $('.data-result').html('<em>可选择多项</em>');
    }
    if ($(DATA_CITY_INPUT).data("value") != "") {
        var inputarry = $(DATA_CITY_INPUT).data("value").split('-');
        var inputarryname = $(DATA_CITY_INPUT).val().split('-');
...
function selectProvince(type, con, isremove) {
    var dataarrary = CITIES_DATA.id_name_map;
    //显示省级
    var strVarCity = "";
    if (type == "all") {
        var dataCityxz      = CITIES_DATA.panel_content.provinces;
        var datahotcityxz   = CITIES_DATA.panel_content.hotcities;
        // 加载热门城市和省份
        ...
        $('.data-result span').each(function (index) {
            if ($('a[data-code=' + $(this).data("code") + ']').length > 0) {
                ...
                if ($('a[data-code=' + $(this).data("code") + ']').attr("class").indexOf('data-all') > 0) {
                    ...
                    if ($('.data-list-provinces a[data-code=' + $(this).data("code").toString().substring(0, 2) + '0000]').length > 0) {
                        var numlabel = $('.data-list-provinces a[data-code=' + $(this).data("code").toString().substring(0, 2) + '0000]').find('label').text();
                        $('.data-list-provinces a[data-code=' + $(this).data("code").toString().substring(0, 2) + '0000]').find('label').text(parseInt(numlabel) + 1).show();
                    }
                }
            } else {
                var numlabel = $('.data-list-provinces a[data-code=' + $(this).data("code").toString().substring(0, 2) + '0000]').find('label').text();
    ...
    //显示下一级
        ...
        // var dataCityxz = CITIES_DATA.panel_content.provinces;
        var relations  = CITIES_DATA.relations;
...
function selectitem_area(con) {
    if (DATA_INPUT_TYPE == "duoxuan") {
        ...
        if ($('.data-result span').length > 2000) {
            ...
            $('.data-result span').each(function () {
                if (($(this).data("code").toString().substring(0, 2) == $(con).data("code").toString().substring(0, 2)) && $(con).data("code").toString().substring(2, 6)== "0000") {
                    $(this).remove();
                }
            })
            $(con).parent('li').siblings('li').find("a").removeClass("d-item-active");
...
function svae_City() {
    ...
    $(DATA_CITY_INPUT).data("value", val);
    $(DATA_CITY_INPUT).val(Cityname);

var auto_area = {
    ...
    appRunList: function (runList, v) {//自动搜索应用
        ...
        //搜索以关键词开头的应用
        for (i in SEARCH_VALUE_CITY) {
            if (isNaN(i) || loaded[i] || !SEARCH_VALUE_CITY[i].name) {
                continue;
            }
            runSearchCode = SEARCH_VALUE_CITY[i].code
            runSearchName = SEARCH_VALUE_CITY[i].name;

            if (runSearchName.indexOf(v) >= 0) {
                loaded[i] = 1;

                var text = runSearchName.replace(v, "<b>" + v + "</b>");
                temp +=
                    '<a class="area_menu" href="javascript:;" data-flag=1 data-code="' + runSearchCode + '" data-name="' + runSearchName + '" onclick="selectProvince(\'sub\',this,\'\')">' +
                    /*'<em>' + runSearchName.replace(v, "<b>" + v + "</b>") + '</em>'*/
                    text + '</a>';
